...
import { fetchSkill, fetchSkillsCatalog, recordSkillInstall, resolveSkillPath } from './api.js'
...
export async function runCli(argv) {
  ...
  // Real install signal for Alpaki counters (capped so a slow API never hangs the CLI).
  await Promise.race([
    recordSkillInstall(skill.id, {
      apiBase: args.api,
      slug: skill.slug || skillPath.slug,
      path: skill.sourcePath || skillPath.path,
    }),
